# Use this to find the r critical value based on Pearson's r
# It first finds the corresponding t critical, then converts to r.
...
def t_critical_tail(alpha, tail):
    # Same logic as your original program, but df = v = n - 2
    if tail == 1:
        # left-tailed: P(T <= t_crit) = alpha
        target = alpha
        low = -10.0
        high = 0.0
        while high - low > 1e-5:
    ...
    if tail == 2:
        # two-tailed: P(-t_crit <= T <= t_crit) = 1 - alpha
        target = 1.0 - alpha
        low = 0.0
        high = 10.0
        while high - low > 1e-5:
    ...
    if tail == 3:
        # right-tailed: P(T <= t_crit) = 1 - alpha
        target = 1.0 - alpha
        low = 0.0
        high = 10.0
        while high - low > 1e-5:
...
def r_critical_tail(alpha, tail):
    # Find t-critical, then convert to r-critical
    tcrit = t_critical_tail(alpha, tail)
    rcrit = tcrit / math.sqrt(tcrit * tcrit + v)
    return round(rcrit, 3)


def main():
    global v
    n = float(input("Sample size n: "))
    v = n - 2  # df for correlation
    ...
    print(r_critical_tail(alpha, tail))
